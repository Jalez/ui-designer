...
type AwarenessLike = {
  doc: Y.Doc;
  on: (event: "change", listener: (payload: { added: number[]; updated: number[]; removed: number[] }) => void) => void;
  off: (event: "change", listener: (payload: { added: number[]; updated: number[]; removed: number[] }) => void) => void;
...
export class YRemoteSelectionsPluginValue {
  conf: { ytext: Y.Text; awareness: AwarenessLike };
  _listener: (payload: { added: number[]; updated: number[]; removed: number[] }) => void;
  _awareness: AwarenessLike;
  decorations: cmView.DecorationSet;

  constructor(view: cmView.EditorView) {
    this.conf = view.state.facet(ySyncFacet) as { ytext: Y.Text; awareness: AwarenessLike };
  ...
  update(update: cmView.ViewUpdate) {
    ...
    const localAwarenessState = awareness.getLocalState() as AwarenessState | null;
    ...
    awareness.getStates().forEach((state, clientid) => {
      ...
      const { color = "#30bced", name = "Anonymous" } = typedState.user || {};
      const colorLight = typedState.user?.colorLight || `${color}33`;
