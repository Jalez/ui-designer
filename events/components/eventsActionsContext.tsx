...
export function useEventsActions(): EventsActionsValue {
  ...
  const handleStartScenarioEventRun = useCallback(() => {
    ...
    if (store.getRuntimeState(selectedRuntimeKey).autoReplay?.running) {
      return;
    }
    store.clearQueuedAutoReplayRequest();
    store.startAutoReplay(
      selectedRuntimeKey,
      selectedScenarioSequence.length + 1,
      store.getSelectedStepIdForScenario(currentLevel, selectedScenario.scenarioId),
    );
